...
def angle_diff(a, b): #to resolve the issue of clockwise/counterclock wise rotation for same orientation, to find the shortest route
    a = normalize(a)
    b = normalize(b)
    d1 = a - b
    d2 = 2 * pi - fabs(d1)
...
class OdometryMotionModel(Node):
    def __init__(self):
        ...
        self.declare_parameter('alpha1', 0.1)  #odometry uncertainty, alphas are ros parameters for OU.
        ...
        if self.nr_samples >= 0:
            self.samples = PoseArray()  #A collection/list of Pose messages.
            self.samples.poses = [Pose() for _ in range(self.nr_samples)]   #Creates Pose message.
        else:
            self.get_logger().fatal('Invalid Number of Samples requested: %d! Exit..', self.nr_samples)
            return

        self.odom_sub = self.create_subscription(
            Odometry, 'franky_controller/odom', self.odom_callback, 10
        )
    ...
    def odom_callback(self, odom): 
        ...
        if sqrt(pow(odom_y_increment, 2) + pow(odom_x_increment, 2)) < 0.01:
            ...
            #which direction the robot translated.
            delta_rot1 = angle_diff(atan2(odom_y_increment, odom_x_increment), yaw)

        delta_trans = sqrt(pow(odom_x_increment, 2) + pow(odom_y_increment, 2))  #previous position ●----distance trvlld----● current position
        ...
        self.pose_array_pub.publish(self.samples)  # Publish samples
